...
import { ContentTypeEnum } from '@/config/common';
import { useAdminUserStore, useUserStore } from '@/store';
...
const host = env === 'development' ? import.meta.env.VITE_API_URL : window.location.origin;
// 数据处理，方便区分多种处理方式
const transform: AxiosTransform = {
  ...
  requestInterceptors: (config, options) => {
    ...
    // userStore为默认应用
    const appStore = app === 'admin' ? adminUserStore : userStore;
    const { accessToken: token } = appStore;
  ...
  responseInterceptorsCatch: async (error: any, instance: AxiosInstance) => {
    ...
    if (error.code === '"ERR_NETWORK"' || !error.response) {
      MessagePlugin.error('网络错误，请检查服务是否开启！');
      return Promise.reject(error);
    }
    // 如果 error 存在 response，说明是业务接口返回的错误
    if (error.response) {
      const { data, status } = error.response;
      const { msg } = data;
      if (status === 403) {
        MessagePlugin.error(msg + error.response.config.url);
      }
      if (status === 404) {
