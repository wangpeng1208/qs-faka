<template>
  <t-card :title="pageName" class="basic-container" :bordered="false">
    ...
    <t-enhanced-table ref="tableRef" v-model:expanded-tree-nodes="expandedTreeNodes" row-key="id" :data="lists" :columns="columns" :tree="treeConfig" :hover="lists.length > 0 ? true : false" max-height="auto" table-layout="auto" :header-affixed-top="headerAffixedTop" :tree-expand-and-fold-icon="treeExpandIcon" lazy-load>
      <template #icon="{ row }">
        <icon-font :name="`t-icon t-icon-${row.icon}`" :url="CDN_CSS" />
      </template>
      <template #hidden="{ row }">
      ...
      <template #operate="{ row }">
        <t-space>
          <t-link v-perms="['adminapi/permission/menu/add']" theme="primary" hover="color" size="small" @click="editRow(0, row.id)">添加</t-link>
          <t-link v-perms="['adminapi/permission/menu/edit']" theme="primary" hover="color" size="small" @click="editRow(row.id)">编辑</t-link>
          <t-popconfirm content="确认删除吗" @confirm="deleteRow(row.id)">
            <t-link v-perms="['adminapi/permission/menu/del']" theme="danger" hover="color" size="small"> 删除 </t-link>
          </t-popconfirm>
        </t-space>
      </template>
    </t-enhanced-table>
...
<script setup lang="ts" name="PermissionMenuIndex">
