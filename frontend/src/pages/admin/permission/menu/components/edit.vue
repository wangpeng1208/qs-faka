<template>
  <t-dialog v-model:visible="visible" :close-on-overlay-click="false" :destroy-on-close="true" width="500" :on-confirm="onSubmit" confirm-on-enter>
    <template #header>
    ...
    <t-form ref="formRef" label-align="left" :data="formData" :rules="FORM_RULES" :label-width="80" @submit="onSubmit">
      ...
      <t-form-item v-if="formData.type === 'L' || formData.type === 'M'" label="菜单图标" name="icon">
        <t-input v-model="formData.icon" clearable placeholder="请选择菜单图标">
          <template #prefix-icon>
            <icon-font :name="`t-icon t-icon-${formData.icon}`" :url="CDN_CSS" style="margin-right: 8px" />
          </template>
        </t-input>
      ...
      <t-form-item v-if="formData.type != 'B'" label="菜单排序" name="sort" help="数值越小越排前">
...
<script setup lang="ts" name="PermissionMenuEdit">
...
import { add, detail, edit, menuTreeSimple } from '@/api/admin/permission/menu';
